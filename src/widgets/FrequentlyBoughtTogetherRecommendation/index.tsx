...
const ProductListViewItem = ({ product, displayPlusItem, isCurrentProduct, onProductClick }: ProductListViewItemProps): JSX.Element => {
...
const PurchaseTogetherListView = ({ products, productsToRemove, currentSku, onProductClick }: PurchaseTogetherListViewProps ): JSX.Element => {
...
interface PurchaseTogetherCheckboxListProps {
  ...
  currentSku: string;
  onProductClick: (data: { sku: string }) => void;
  onCheckboxClick: (sku: string) => void;
}
const PurchaseTogetherCheckboxList = ({ products, productsToRemove, currentSku, onProductClick, onCheckboxClick }: PurchaseTogetherCheckboxListProps ): JSX.Element => {
  const navigate = useNavigate();
  return (
...
export const FrequentlyBoughtTogetherRecommendation = (): JSX.Element => {
  const {
  ...
  const [productsToRemove, setProductsToRemove] = useState<Record<string, boolean>>({});

  const onCheckboxClick = (sku: string) => {
